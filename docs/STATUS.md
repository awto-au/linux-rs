# Status — 2026-08-02T20:45:30+10:00

![status](status/status.png)

- Translated TUs: **41** (**41 hand** — bridge, tracked for transpiler replacement · **0 c2rust**)   ·   KUnit: **20 suites, 224 vectors** green   ·   Rules: **32** (t1 21 / t2 6 / t3 5)
- Wired into live boot path: **6 functions across 1 file(s)** (vs. **42** total TUs landed) — see [docs/streams.md](streams.md)'s stream 2 ("c2rust-boot-blocker") for why this is the harder, more important milestone: a standalone `lib/` swap compiling clean is not the same as real Rust executing at a live, pre-existing C call site.
...
| Whole-file `lib/` swap (Makefile points straight at `*_rs.rs`, no call-site wrapper) | **42** | `*_rs.rs` TUs whose sibling `.c` (if any) has no `#ifdef CONFIG_RUST` wrapper calling into it |
...
| TU | readiness |
|---|---|
| lib/zlib_inflate/inffast.c | 94.7% |
| lib/zlib_inflate/inftrees.c | 93.5% |
| lib/decompress_bunzip2.c | 89.3% |
| lib/lzo/lzo1x_decompress_safe.c | 85.3% |
| lib/zlib_inflate/infutil.c | 85.2% |
| lib/string_helpers.c | 85.0% |
| lib/xz/xz_dec_stream.c | 84.7% |
| lib/kfifo.c | 84.5% |
| lib/zlib_inflate/inflate.c | 82.7% |
| lib/decompress_unlzo.c | 81.8% |
