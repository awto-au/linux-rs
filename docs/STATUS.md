# Status — 2026-07-31T17:29:37+10:00

![status](status/status.png)

- Translated TUs: **38**   ·   KUnit: **19 suites, 160 vectors** green   ·   Rules: **31** (t1 21 / t2 5 / t3 5)
- Wired into live boot path: **6 functions across 1 file(s)** (vs. **39** total TUs landed) — see [docs/streams.md](streams.md)'s stream 2 ("c2rust-boot-blocker") for why this is the harder, more important milestone: a standalone `lib/` swap compiling clean is not the same as real Rust executing at a live, pre-existing C call site.
...
| Whole-file `lib/` swap (Makefile points straight at `*_rs.rs`, no call-site wrapper) | **39** | `*_rs.rs` TUs whose sibling `.c` (if any) has no `#ifdef CONFIG_RUST` wrapper calling into it |
